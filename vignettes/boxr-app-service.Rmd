...
3. The `dir_id` of the folder you want to share.
...
```r
box_dir_invite("91350609301", user_id = "286871517", role = "co-owner")
```
...
TODO: get new four-screens image
...
Leave the dangerous stuff alone.

Note the `client_id`

  This action requies two-step authentication. If two-step authentication is not enabled on your Box account, you will need to enable it to proceed.

1. In the **Add and Manage Public Keys** menu, click **Generate a Public/Private Keypair**. 
...
The last step is to authorize your app - this has to be done by someone on your Box-admin team. If this is a personal account, that's you.

1. Using the account dropdown-menu in the upper-left, go to the **Admin Console**.

2. From the sidebar menu on the left, select **Enterprise Settings**.

3. From the top tab-bar, select **Apps**.

4. Scroll down to the **Custom Application** menu, click **Authorize New App**.

Enter the `client_id` for your app (which you saved from above). This also can be found in the your token-file, e.g. `token.json`.
