...
There are functions to transfer files:

 - `box_dl()`, `box_ul()`: download or upload individual files

There are functions to get information about a file's version history:
...
That said, directories are identified using a `dir_id`:
...
### Interacting with Box files
  ...
  - `box_collab_create()`, `box_collab_get()`, `box_collab_delete()`
  - `box_comment_create()`, `box_comment_get()`
  - `box_add_description()`

### Using Box trash

  - `box_delete_file()`, `box_restore_file()`:
  - `box_delete_folder()`, `box_restore_folder()`:
